...
<div class="sub-udiv card1" style="width: 98%; height: 430px;">
<table class="table1">
	<tr>
		<th colspan="9">Your Ticket Booked History.</th>
	</tr>
	...
	try{
		PreparedStatement ps=con.prepareStatement("select *from ticket_table where user_id=?");
		...
		while(rs.next()){
			...
			if(rs.getString(10).equals("Confirm")){
				%>
				<td class="td-c"><input type="submit" value="Cancel" onclick="cancleprocess1('<%=rs.getString(1) %>','<%=rs.getInt(5) %>','<%=obj.datestr(rs.getString(6)) %>','<%=rs.getString(7) %>')" class="button"></td>
